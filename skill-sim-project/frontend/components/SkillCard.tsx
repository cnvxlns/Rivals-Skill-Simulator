...
const SkillCard = ({ skillSlot, slotNumber, className }: Props) => {
  ...
  const primaryDescription = skill.description?.trim() || skill.effects?.find((effect) => effect.description)?.description || '';
  const hasEffectDetails = (skill.effects?.length ?? 0) > 0;

  return (
    <div
      title={primaryDescription || undefined}
    ...
    >
      ...
      {(primaryDescription || hasEffectDetails) && (
        <div className="mt-3 rounded-lg border border-indigo-100/80 bg-indigo-50/80 p-3 text-sm text-slate-800 shadow-inner">
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-indigo-700">In-Game Description</p>
            <button
              type="button"
              onClick={() => setShowDescription((prev) => !prev)}
              className="text-[11px] font-semibold uppercase tracking-wide text-indigo-700 underline decoration-indigo-300 decoration-2 underline-offset-4"
            >
          ...
          {showDescription && (
            <div className="mt-2 space-y-2">
              {primaryDescription ? (
                <p className="whitespace-pre-line text-[13px] leading-relaxed text-slate-900">{primaryDescription}</p>
              ) : (
                <p className="text-[12px] text-slate-600">No description available from API.</p>
              )}
